...
public class IOSNativeSettingsEditor : Editor{
	private static string[] ToolbarHeaders = new string[] {
		"General",
		"Billing",
		"Game Center",
		"Other"
	...
	public static void CameraAndGallery(bool showTitle = true){if(showTitle){
			EditorGUILayout.Space();
			...
			if(IOSNativeSettings.Instance.ShowLeaderboards){
				...
				foreach(GK_Leaderboard leaderboard in IOSNativeSettings.Instance.Leaderboards){
					...
					if(leaderboard.Info.Texture != null){
						GUILayout.Box(leaderboard.Info.Texture, ImageBoxStyle, new GUILayoutOption[] {
							GUILayout.Width(18),
							GUILayout.Height(18)
						});
					}
					...
					if(leaderboard.IsOpen){
						...
						EditorGUILayout.BeginHorizontal();
						leaderboard.Info.Description = EditorGUILayout.TextArea(leaderboard.Info.Description, new GUILayoutOption[] {
							GUILayout.Height(60),
							GUILayout.Width(200)
						});
			...
			if(IOSNativeSettings.Instance.ShowAchievementsParams){
				...
				foreach(GK_AchievementTemplate achievement in IOSNativeSettings.Instance.Achievements){
					...
					if(achievement.Texture != null){
						GUILayout.Box(achievement.Texture, ImageBoxStyle, new GUILayoutOption[] {
							GUILayout.Width(18),
							GUILayout.Height(18)
						});
					}
			...
			if(IOSNativeSettings.Instance.ShowStoreKitProducts){
				foreach(IOSProductTemplate product in IOSNativeSettings.Instance.InAppProducts){
					...
					if(product.Texture != null){
						GUILayout.Box(product.Texture, ImageBoxStyle, new GUILayoutOption[] {
							GUILayout.Width(18),
							GUILayout.Height(18)
						});
					}
	...
	public static void DisableAdditionalFeatures(){
		ChangeDefineState(ISN_ReplayKit_Path, "REPLAY_KIT", false);
		ChangeDefineState(ISN_CloudKit_Path, "CLOUD_KIT", false);
		ChangeDefineState(IOSNotificationController_Path, "PUSH_ENABLED", false);
		ChangeDefineState(DeviceTokenListener_Path, "PUSH_ENABLED", false);
	}
	...
	public static void UpdatePluginSettings(){
		...
		ChangeDefineState(IOSNotificationController_Path, "PUSH_ENABLED", IOSNativeSettings.Instance.EnablePushNotificationsAPI);
		ChangeDefineState(DeviceTokenListener_Path, "PUSH_ENABLED", IOSNativeSettings.Instance.EnablePushNotificationsAPI);
		ChangeDefineState(GameCenterManager_Path, "GAME_CENTER_ENABLED", IOSNativeSettings.Instance.EnableGameCenterAPI);
		ChangeDefineState(GameCenter_TBM_Path, "GAME_CENTER_ENABLED", IOSNativeSettings.Instance.EnableGameCenterAPI);
		ChangeDefineState(GameCenter_RTM_Path, "GAME_CENTER_ENABLED", IOSNativeSettings.Instance.EnableGameCenterAPI);
		ChangeDefineState(IOSNativeMarketBridge_Path, "INAPP_API_ENABLED", IOSNativeSettings.Instance.EnableInAppsAPI);
		ChangeDefineState(IOSStoreProductView_Path, "INAPP_API_ENABLED", IOSNativeSettings.Instance.EnableInAppsAPI);
		ChangeDefineState(ISN_Security_Path, "INAPP_API_ENABLED", IOSNativeSettings.Instance.EnableInAppsAPI);
		ChangeDefineState(iAdBannerControllerr_Path, "IAD_API", IOSNativeSettings.Instance.EnableiAdAPI);
		ChangeDefineState(iAdBanner_Path, "IAD_API", IOSNativeSettings.Instance.EnableiAdAPI);
		ChangeDefineState(IOSSocialManager_Path, "SOCIAL_API", IOSNativeSettings.Instance.EnableSocialSharingAPI);
		ChangeDefineState(IOSCamera_Path, "CAMERA_API", IOSNativeSettings.Instance.EnableCameraAPI);
		ChangeDefineState(IOSVideoManager_Path, "VIDEO_API", IOSNativeSettings.Instance.EnableMediaPlayerAPI);
		ChangeDefineState(ISN_MediaController_Path, "VIDEO_API", IOSNativeSettings.Instance.EnableMediaPlayerAPI);
		ChangeDefineState(ISN_ReplayKit_Path, "REPLAY_KIT", IOSNativeSettings.Instance.EnableReplayKit);
		ChangeDefineState(ISN_CloudKit_Path, "CLOUD_KIT", IOSNativeSettings.Instance.EnableCloudKit);
		...
		if(!IOSNativeSettings.Instance.EnableSoomla){
			ChangeDefineState(ISN_Soomla_Path, "SOOMLA", IOSNativeSettings.Instance.EnableSoomla);
	...
	private static void ChangeDefineState(string file, string tag, bool IsEnabled){
