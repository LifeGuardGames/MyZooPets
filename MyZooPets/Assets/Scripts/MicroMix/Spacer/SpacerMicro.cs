...
public class SpacerMicro : Micro{
	...
	protected override void _StartMicro(int difficulty, bool randomize){
		...
		if(randomize){
			do{
				inhaler.transform.position = CameraUtils.RandomWorldPointOnScreen(Camera.main, .1f, .1f);
				spacer.transform.position = CameraUtils.RandomWorldPointOnScreen(Camera.main, .2f, .2f);
